...
class Webservice extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        fwrite($fp,"Currunt URL: ".$_SERVER['REDIRECT_REQUEST_METHOD'].": ".$fullURL."\n\r");
    ...
    public function handle_upload_file()
    {
        ...
        if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
            ...
            $allowed_extension = array_map('trim', array_map('strtolower',  array('gif', 'jpg', 'jpeg', 'jpe', 'png')));
    ...
    public function handle_upload_file_compulsory()
    {
        ...
        if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
            ...
            $ext               = strtolower(pathinfo($file_name, PATHINFO_EXTENSION));
            if ($files = filesize($_FILES['file']['tmp_name'])) {
                // die($file_type." | ". $allowed_mime_type);
                // if (!in_array($file_type, $allowed_mime_type)) {
                    ...
                    foreach ($respdata as $key => $value) {
                        ...
                        if (!empty($question)) {
                         ...
                         if($value->time_from == null){
                            $value->time_from = date("h:i:s",time()+3600);
                        }
                        if($value->time_to == null){
                            $value->time_to = date("h:i:s",time()+7200);
                        }
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if (!empty($exam_result->exam_result)) {
                        ...
                        $exam->exam_credit_hour               = 0;
                        $exam->exam_result_status             = "Not Applicable"; // pass renamed to - 
                        if ($exam_result->exam_result['exam_connection'] == 0) {
                            ...
                            foreach ($exam_result->exam_result['result'] as $exam_result_key => $exam_result_value) {
                                ...
                                if ($exam_result_value->attendence != "present") {
                                    $exam->exam_result_status = "Not Applicable"; // fail renamed to - 
                            ...
                            $consolidate_result_status              = "Not Applicable"; // pass renamed to - 
                            if (!empty($exam_result->exam_result['exams'])) {
                                $consolidate_exam_result = "Not Applicable"; // pass renamed to - 
                                foreach ($exam_result->exam_result['exams'] as $each_exam_key => $each_exam_value) {
                                    if ($exam_result->exam_type != "gpa") {
                                        ...
                                        if ($consolidate_each->exam_status == "fail") {
                                            $consolidate_result_status = "Not Applicable"; // fail renamed to - 
                                        }

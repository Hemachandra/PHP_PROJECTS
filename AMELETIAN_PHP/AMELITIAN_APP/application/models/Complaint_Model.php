...
class complaint_Model extends MY_Model {
    ...
    public function complaint_list($id = null) {
        if ($id != null) {
            $this->db->select("complaint.*,students.admission_no,students.firstname,students.lastname,classes.class,sections.section,students.guardian_name,students.guardian_phone,staff.name,staff.surname")
            ->from('complaint');
            $this->db->join("students","students.admission_no = complaint.admission_no","left");
            ...
            $this->db->join('staff', 'complaint.assigned = staff.id',"left");

            $this->db->where('complaint.id', $id);
        } else {
            $this->db->select("complaint.*,staff.name as firstname,staff.surname,students.id as student_id")->from('complaint');
            $this->db->join("students","students.admission_no = complaint.admission_no","left");
            $this->db->join('staff', 'complaint.assigned = staff.id',"left");

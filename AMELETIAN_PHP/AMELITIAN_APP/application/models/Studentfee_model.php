...
class Studentfee_model extends CI_Model {
    ...
    public function remove($id, $sub_invoice,$reason='') {
        $this->db->where('id', $id);
        $q = $this->db->get('student_fees_deposite');
        if ($q->num_rows() > 0) {
            $result = $q->row();
            $a = json_decode($result->amount_detail, true);
            $storeInfo = json_encode($a[$sub_invoice]);
            ...
            $staff_id = $session['id'];
            //fee_revent_cancel
            $fee_revent_cancel = array(
                'details' => $storeInfo,
                'user_id' => $staff_id,
                'reason' => $reason
            );
            $this->db->insert("fee_revent_cancel",$fee_revent_cancel);
        }
    }

...
class Student_model extends MY_Model
{
    ...
    public function searchByClassSection($class_id = null, $section_id = null,$hostel_id=null,$hostel_room_id=null,$vehroute_id=null)
    {
    ...
    public function getPastSessionStudent($previous_session_id, $class_id, $section_id)
    {
        $sql = "SELECT student_session.student_id, student_session.id, student_session.class_id,sessions.id, `sessions`.`session`, ";
        $sql .= " students.firstname,students.middlename,students.lastname,students.admission_no,students.roll_no,";
        $sql .= " students.father_name,students.admission_date ";
        $sql .= " FROM student_session  ";
        $sql .= " INNER join students on students.id =student_session.student_id  ";
        $sql .= " LEFT JOIN sessions ON sessions.id = student_session.session_id ";
        $sql .= " WHERE student_session.session_id= student_session.session_id ";
        if($class_id){
            $sql .=" and student_session.class_id=$class_id ";
        }
        if($section_id){
    ...
    public function lastRecord2()
    {
        $last_row = $this->db->select('*')->like(array('admission_no' => "MES"))->order_by('id', "desc")->limit(1)->get('students')->row();
        return $last_row;
    }
    ...
    public function reportClassSection($class_id = null, $section_id = null,$previous = false,$from_date=false,$to_date=false)
    {
        ...
        $this->db->select('classes.id AS `class_id`,student_session.id as student_session_id,students.id,classes.class,sections.id AS `section_id`,sections.section,students.id,students.mother_phone,students.mother_name,students.admission_no,students.roll_no,students.admission_date,students.firstname,students.middlename,students.lastname,students.image,students.mobileno, students.email ,students.state,students.city,students.pincode,students.religion,students.dob ,students.current_address,students.permanent_address,IFNULL(students.category_id, 0) as `category_id`,IFNULL(categories.category, "") as `category`,students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code , students.guardian_name , students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.father_name,students.rte,students.gender,' . $field_variable)->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
        $this->db->join('categories', 'students.category_id = categories.id', 'left');

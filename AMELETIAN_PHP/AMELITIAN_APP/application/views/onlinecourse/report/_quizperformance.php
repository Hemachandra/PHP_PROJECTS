	...
	<div class="col-md-8">
								<div class="scroll-area-fullheight">
									<?php
										...
										if (!empty($quizperformancedata)) {  
									?>
									<?php foreach ($quizperformancedata as $quizperformancedata_value) {
										?> 
						<?php
						 //$chacknotempty	= $quizgraphdata[$totalquiz_value['id']]['total_question']; 
									//if($chacknotempty >= 1){
										$count++;
										  ?>
		...
		<canvas id="doughnuts-chart_<?php echo $chart_count; ?>" ></canvas>
													</div>
												</div>							
												</div>
											</div>

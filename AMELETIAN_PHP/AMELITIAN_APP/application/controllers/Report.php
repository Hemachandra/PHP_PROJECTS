...
class Report extends Admin_Controller
{
    ...
    public function fee_cancellation(){
        $data = array();
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'Reports/finance');
        $this->session->set_userdata('subsub_menu', 'Reports/finance/fee_cancellation');
        $data['reports'] = $this->db->select("fee_revent_cancel.*,CONCAT(staff.name, ' ',staff.surname) as added_by")->from("fee_revent_cancel")->join("staff","staff.id = fee_revent_cancel.user_id")->order_by("id","desc")->get()->result();
        $this->load->view('layout/header', $data);
        $this->load->view('reports/fee_cancellation', $data);
        $this->load->view('layout/footer', $data);
    }

...
class Student extends Admin_Controller
{
    ...
    public function view($id)
    {
        ...
        $data['sch_setting']     = $this->sch_setting_detail;
        $data['adm_auto_insert'] = $this->sch_setting_detail->adm_auto_insert;
        $current_student_session = $this->student_model->get_studentsession(isset($student['student_session_id'])?($student['student_session_id']):(0));
        $data["session"]              = $current_student_session["session"];
        $student_due_fee              = $this->studentfeemaster_model->getStudentFees(isset($student['student_session_id'])?($student['student_session_id']):(0));
        $student_discount_fee         = $this->feediscount_model->getStudentFeesDiscount(isset($student['student_session_id'])?($student['student_session_id']):(0));
    ...
    public function create()
    {
        ...
        // die(print_r($this->sch_setting_detail));
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($this->sch_setting_detail->adm_auto_insert) {
                if ($this->sch_setting_detail->adm_update_status) {
                    ...
                    $last_student         = $this->student_model->lastRecord2();
                ...
                if ($admission_no_exists) {
                    // $insert = false;
                     ...
                     $last_admission_digit = str_replace($this->sch_setting_detail->adm_prefix, "", $admission_no);
                    $admission_no                = $this->sch_setting_detail->adm_prefix . sprintf("%0" . $this->sch_setting_detail->adm_no_digit . "d", $last_admission_digit + 1);
                    $data_insert['admission_no'] = $admission_no;
            ...
            if ($insert) {
                ...
                $data_student_login = array(
                    ...
                    'password' => $this->input->post('mobileno'),
                    'user_id'  => $insert_id,
                    'role'     => 'student',
                );
                ...
                if ($sibling_id > 0) {
                    ...
                    $data_parent_login = array(
                        ...
                        // 'username' => $this->generateSeoURL($this->input->post('father_name')),
                        'username' => (($this->input->post('guardian_phone')) && $this->input->post('guardian_phone') != "")?($this->input->post('guardian_phone')):($this->input->post('father_phone')),
                        'password' => $this->input->post('father_phone'),
                        'user_id'  => 0,
                        'role'     => 'parent',
                        'childs'   => $temp,
                    );
                    ...
                    if (!empty($result)) {
                        ...
                        for ($i = 1; $i <= count($result); $i++) {
                            ...
                            if ($this->form_validation->is_unique($adm_no, 'students.admission_no')) {
                                if(isset($student_data[$i][""])){
                                    unset($student_data[$i][""]);
                                }
                           $insert_id = $this->student_model->add($student_data[$i], $data_setting);
                            ...
                            if (!empty($insert_id)) {
                                ...
                                $data_student_login = array(
                                    // 'username' => $this->student_login_prefix . $insert_id,
                                    // 'password' => $user_password,
                                    'username' => $student_data[$i]["admission_no"],
                                    'password' => $student_data[$i]["mobileno"],
                                    'user_id'  => $insert_id,
                                    'role'     => 'student',
                                );
                                ...
                                $data_parent_login = array(
                                    // 'username' => $this->parent_login_prefix . $insert_id,
                                    'username' => (isset($student_data[$i]["guardian_phone"]) && $student_data[$i]["guardian_phone"] != "")?($student_data[$i]["guardian_phone"]):($this->student_login_prefix . $insert_id),
                                    'password' => $parent_password,
                                    'user_id'  => $insert_id,
                                    'role'     => 'parent',
                                    'childs'   => $temp,
                                ...
                                $this->session->set_flashdata('msg', '<div class="alert alert-danger text-center">'.' admission #' . $adm_no . $this->lang->line('record_already_exists') . '</div>');
                            }
                        }
                    } else {
    ...
    public function submit_move_student(){
        $old_section = $this->input->post("old_section");
        ...
        foreach($ids as $key => $val){
            if($val != ""){
                $this->db->set("section_id",$new_section)->where("class_id",$class_id)->where("section_id",$old_section)->where("student_id",$val)->update("student_session");
            }
        ...
        echo json_encode(
            array("status" => "success","message" => "Students moved successfully!","")
        );
    }

    public function submit_move_hostel(){
        ...
        $hostel_id2 = $this->input->post("hostel_id2");
        $hostel_room_id2 = $this->input->post("hostel_room_id2");

        $ids = explode(",",$this->input->post("ids"));
        
        foreach($ids as $key => $val){
            if($val != ""){
                $this->db->set("hostel_room_id",$hostel_room_id2)
                ->where("id",$val)
                ->update("students");
            }
        }
    ...
    public function submit_move_transport(){
        $vehroute_id = $this->input->post("vehroute_id");
        ...
        foreach($ids as $key => $val){
            if($val != ""){
                $this->db->set("vehroute_id",$vehroute_id)
                ->where("id",$val)
                ->update("students");
            }
        }
    ...
    public function move_student()
    {
        ...
        $this->session->set_userdata('top_menu', 'Student Information');
        $this->session->set_userdata('sub_menu', 'student/move_student');
        ...
        if ($this->input->server('REQUEST_METHOD') == "GET") {
            $this->load->view('layout/header', $data);
            $this->load->view('student/moveStudent', $data);
            $this->load->view('layout/footer', $data);
        } else {
            $class       = $this->input->post('class_id');
            $section     = $this->input->post('section_id');
            ...
            if (isset($search)) {
                if ($search == 'search_filter') {
                    ...
                    if ($this->form_validation->run() == false) {
                        ...
                        $data['resultlist']  = $resultlist;
                        $title               = $this->classsection_model->getDetailbyClassSection($data['class_id'], $data['section_id']);
                        $cls = isset($title['class'])?($title['class']):('');
                        $sec = isset($title['section'])?($title['section']):('');
                        $data['title']       = 'Student Details for ' . $cls . "(" . $sec . ")";
                    }
    ...
    public function move_hostel()
    {
        ...
        $this->session->set_userdata('top_menu', 'Student Information');
        $this->session->set_userdata('sub_menu', 'student/move_hostel');
        $data['title']           = 'Student Search';
        $data['adm_auto_insert'] = $this->sch_setting_detail->adm_auto_insert;
        $data['sch_setting']     = $this->sch_setting_detail;
        $data['fields']          = $this->customfield_model->get_custom_fields('students', 1);
        ...
        if ($this->input->server('REQUEST_METHOD') == "GET") {
            $this->load->view('layout/header', $data);
            $this->load->view('student/moveHostel', $data);
            $this->load->view('layout/footer', $data);
        } else {
            $class       = $this->input->post('class_id');
            $section     = $this->input->post('section_id');
            ...
            if (isset($search)) {
                if ($search == 'search_filter') {
                    ...
                    if ($this->form_validation->run() == false) {
                        ...
                        $data['section_id']  = $this->input->post('section_id');
                        $data['search_text'] = $this->input->post('search_text');
                        $resultlist          = $this->student_model->searchByClassSection($class, $section,$hostel_id,$hostel_room_id);
                        $data['resultlist']  = $resultlist;
                        $title               = $this->classsection_model->getDetailbyClassSection($data['class_id'], $data['section_id']);
                        $cls = isset($title['class'])?($title['class']):('');
                        $sec = isset($title['section'])?($title['section']):('');
    ...
    public function move_transport()
    {
        if (!$this->rbac->hasPrivilege('student', 'can_view')) {
            access_denied();
        }
        ...
        $this->session->set_userdata('top_menu', 'Student Information');
        $this->session->set_userdata('sub_menu', 'student/move_transport');
        $data['title']           = 'Student Search';
        $data['adm_auto_insert'] = $this->sch_setting_detail->adm_auto_insert;
        $data['sch_setting']     = $this->sch_setting_detail;
        $data['fields']          = $this->customfield_model->get_custom_fields('students', 1);
        ...
        if ($this->input->server('REQUEST_METHOD') == "GET") {
            $this->load->view('layout/header', $data);
            $this->load->view('student/moveTransport', $data);
            $this->load->view('layout/footer', $data);
        } else {
            $class       = $this->input->post('class_id');
            $section     = $this->input->post('section_id');
            ...
            if (isset($search)) {
                if ($search == 'search_filter') {
                    ...
                    if ($this->form_validation->run() == false) {
                        ...
                        $data['section_id']  = $this->input->post('section_id');
                        $data['search_text'] = $this->input->post('search_text');
                        $resultlist          = $this->student_model->searchByClassSection($class, $section,null,null,$vehroute_id);
                        $data['resultlist']  = $resultlist;
                        $title               = $this->classsection_model->getDetailbyClassSection($data['class_id'], $data['section_id']);
                        $cls = isset($title['class'])?($title['class']):('');
                        $sec = isset($title['section'])?($title['section']):('');
    ...
    public function profilesetting()
    {
        ...
        $data['result']          = $this->setting_model->getSetting();
        $data['sch_setting_detail']          = (array)$this->sch_setting_detail;

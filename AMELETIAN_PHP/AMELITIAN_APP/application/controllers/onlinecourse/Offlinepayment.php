    ...
    public function courselist()
    {
        ...
        if (!empty($coursedata->data)) {
            ...
            foreach ($coursedata->data as $key => $value) {
                ...
                $value->paidstatus = $this->courseofflinepayment_model->paidstatus($value->id, $studentid);
                ...
                $row[] = $price;
                $row[] = $discount;
                if ($value->paidstatus == 1) {
                    $row[] = '<button data-backdrop="static" data-id=' . $value->id . ' user-data-id=' . $studentid . ' class-section-id=' . $class_section_id . ' class_id=' . $class_id . ' class="btn btn-danger btn-xs pull-right revert_btn"><i class="fa fa-undo"> </i> ' . $this->lang->line("revert") . ' </button> <button data-backdrop="static" data-id=' . $value->id . ' user-data-id=' . $studentid .'  class="btn btn-primary btn-xs pull-right print_btn"><i class="fa fa-print"></i> ' . $this->lang->line("print") . '</button>';
                } else {

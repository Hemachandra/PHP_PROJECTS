...
class Users extends Admin_Controller
{
    ...
    public function updateParentUserName(){
        $data = $this->user_model->updateParentUserName();
        foreach($data as $key => $val){
            if(trim($val['guardian_phone']) != ""){
                $this->db->where("id",$val['id']);
                $this->db->update("users",array("username"=>$val['guardian_phone']));
                // die(print_r($val));
                echo "Old username: ".$val['username'].' | New Username: '.$val['guardian_phone'].'<br />';
            }else{
                $this->db->where("id",$val['id']);
                $this->db->update("users",array("username"=>'parent'.$val['user_id']));
                echo "Old username: ".$val['username'].' | New Username: '.'parent'.$val['user_id'].'<br />';
            }
        }
    }

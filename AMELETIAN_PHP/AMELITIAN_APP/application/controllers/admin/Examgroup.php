...
//====================
    ...
    public function subjectstudent() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $exam_subject_id = $this->input->post('teachersubject_id'); // original subject id
            $subject_id = $this->input->post('subject_id'); // exam subject id
            ...
            $data['section_id'] = $this->input->post('section_id');
            $data['session_id'] = $this->input->post('session_id');
            $resultlist = $this->examgroupstudent_model->examGroupSubjectResultCustom($subject_id, $class_id, $section_id, $session_id);
            $subject_detail = $this->batchsubject_model->getExamSubjects($subject_id);
            
            $data['subject_detail'] = $subject_detail;
            $data['attendence_exam'] = $this->attendence_exam;
            $data['resultlist'] = $resultlist;
            // die(print_r($subject_detail));
            $data['sch_setting']  =$this->sch_setting_detail;
            
            $student_exam_page = $this->load->view('admin/examgroup/_partialstudentmarkEntry', $data, true);
            $exam_subjects = $this->batchsubject_model->getExamSubjects($exam_subject_id);
            $csvData = array();
            $csvData[] = array(
                'adm_no' => 'adm_no',
                'status' => 'status',
            ...
            fclose($fp);
            $student_exam_page .= '<a target="_blank" href="'.base_url().$csvFileName .'">Download CSV File</a>';
    ...
    public function entrymarks() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!empty($exam_group_student_id)) {
                ...
                foreach($exam_group_student_id as $k => $v){
                    foreach ($v as $exam_group_student_key => $exam_group_student_value) {
                        $attendance_post = $this->input->post($k.'exam_group_student_attendance_' . $exam_group_student_value);
                        if (isset($attendance_post)) {
                            $attendance = $this->input->post($k.'exam_group_student_attendance_' . $exam_group_student_value);
                        } else {
                            $attendance = "present";
                        }
                        $array = array(
                            ...
                            'internal_marks' => $this->input->post($k.'_exam_group_student_internal_mark_' . $exam_group_student_value),
                            'external_marks' => $this->input->post($k.'_exam_group_student_external_mark_' . $exam_group_student_value),
                            'get_marks' =>  $this->input->post($k.'_exam_group_student_internal_mark_' . $exam_group_student_value)+$this->input->post($k.'_exam_group_student_external_mark_' . $exam_group_student_value),
                            'note' => $this->input->post($k.'_exam_group_student_note_' . $exam_group_student_value),
                        );
                        $insert_array[] = $array;
                    }
                }
    ...
    public function getexamSubjects() {
        ...
        $data['exam_subjects'] = $this->batchsubject_model->getExamSubjectsCustom($exam_id);
    ...
    public function getSubjectByExam() {
        ...
        $data['exam_subjects'] = $this->batchsubject_model->getExamSubjectsCustom($id);

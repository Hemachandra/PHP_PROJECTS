...
function getProperties($items_per_page, $connect, $lang,$type=''){
    ...
    if($type != ''){
        ...
        $sqlQuery .= " AND properties.pt_id IN (SELECT pe_property FROM properties_extras WHERE pe_extra = $type) ";
    }
    ...
    //echo "<pre>SQL Query: " . htmlspecialchars($sqlQuery) . "</pre>";
...
$lang = checkLanguage($connect, $settings['st_language']);
	...
	$stmt->execute(array(':pt_city_id' => $id_city));
...
function getSearchProperties($items_per_page, $connect, $lang){
    ...
    $sqlQuery = "SELECT SQL_CALC_FOUND_ROWS properties.*, tr_properties.*, tr_ptstatus.tr_name AS status, tr_ptcities.tr_name AS city, tr_ptzones.tr_name AS zone, CAST(properties.pt_price AS UNSIGNED) AS price, CAST(properties.pt_size AS UNSIGNED) AS size, tr_pttypes.tr_name AS type, tr_ptconditions.tr_name AS conditions, tr_properties.tr_slug AS slug FROM properties LEFT JOIN tr_properties ON properties.pt_id = tr_properties.tr_property LEFT JOIN tr_ptstatus ON properties.pt_status = tr_ptstatus.tr_status AND tr_ptstatus.tr_lang = '".$lang."' LEFT JOIN tr_pttypes ON properties.pt_type = tr_pttypes.tr_type AND tr_pttypes.tr_lang = '".$lang."' LEFT JOIN tr_ptcities ON properties.pt_city = tr_ptcities.tr_city AND tr_ptcities.tr_lang = '".$lang."' 
    
    LEFT JOIN tr_ptzones ON properties.pt_zone = tr_ptzones.tr_zone AND tr_ptzones.tr_lang = '".$lang."' 
    
    LEFT JOIN tr_ptmandal ON properties.pt_mandal = tr_ptmandal.tr_mandal AND tr_ptmandal.tr_lang = '".$lang."' 
    LEFT JOIN tr_ptvillage ON properties.pt_village = tr_ptvillage.tr_village AND tr_ptvillage.tr_lang = '".$lang."' 
    
    LEFT JOIN tr_ptconditions ON properties.pt_conditions = tr_ptconditions.tr_conditions AND tr_ptconditions.tr_lang = '".$lang."' WHERE properties.pt_visibility = 1 AND properties.pt_sold = 0 AND tr_properties.tr_lang = '".$lang."'";

    if(getParamsReference() && !empty(getParamsReference())){

        // $sqlQuery .= " AND properties.pt_reference = '".getParamsReference()."'";
        $sqlQuery .= " AND (tr_ptmandal.tr_name like '%".getParamsReference()."%'";
        $sqlQuery .= " OR tr_ptvillage.tr_name like '%".getParamsReference()."%' )";
    }
...
function getSearchPropertieMobile($items_per_page, $connect, $lang, $city, $zone){
...
{
    $sentence = $connect->query("SELECT 
    sliders.*, 
    properties.pt_id, 
    properties.pt_price, 
    properties.pt_direction, 
    properties.pt_size, 
    properties.pt_beds, 
    properties.pt_baths, 
    tr_properties.tr_label, 
    tr_properties.tr_title, 
    tr_properties.tr_slug, 
    tr_ptstatus.tr_name AS status, 
    tr_ptstatus.tr_status, 
    tr_ptstatus.tr_lang, 
    tr_ptcities.tr_name AS city, 
    tr_ptzones.tr_name AS zone
FROM sliders
LEFT JOIN properties ON properties.pt_id = sliders.slider_property
LEFT JOIN tr_properties ON properties.pt_id = tr_properties.tr_property AND tr_properties.tr_lang = '".$lang."'
LEFT JOIN tr_ptstatus ON properties.pt_status = tr_ptstatus.tr_status AND tr_ptstatus.tr_lang = '".$lang."'
LEFT JOIN tr_pttypes ON properties.pt_type = tr_pttypes.tr_type AND tr_pttypes.tr_lang = '".$lang."'
LEFT JOIN tr_ptcities ON properties.pt_city = tr_ptcities.tr_city AND tr_ptcities.tr_lang = '".$lang."'
LEFT JOIN tr_ptzones ON properties.pt_zone = tr_ptzones.tr_zone AND tr_ptzones.tr_lang = '".$lang."'
WHERE slider_visibility = 1
...
function getLatestProperties($connect, $lang, $value){
    $sentence = $connect->prepare("SELECT properties.*, tr_properties.*, tr_ptstatus.tr_name AS status, tr_ptcities.tr_name AS city, tr_ptzones.tr_name AS zone, tr_pttypes.tr_name AS type, tr_ptconditions.tr_name AS conditions, tr_properties.tr_slug AS slug 
    FROM properties 
    LEFT JOIN tr_properties ON properties.pt_id = tr_properties.tr_property 
    LEFT JOIN tr_ptstatus ON properties.pt_status = tr_ptstatus.tr_status AND tr_ptstatus.tr_lang = '".$lang."' 
    LEFT JOIN tr_pttypes ON properties.pt_type = tr_pttypes.tr_type AND tr_pttypes.tr_lang = '".$lang."' 
    LEFT JOIN tr_ptcities ON properties.pt_city = tr_ptcities.tr_city AND tr_ptcities.tr_lang = '".$lang."' 
    LEFT JOIN tr_ptzones ON properties.pt_zone = tr_ptzones.tr_zone AND tr_ptzones.tr_lang = '".$lang."' 
    LEFT JOIN tr_ptconditions ON properties.pt_conditions = tr_ptconditions.tr_conditions AND tr_ptconditions.tr_lang = '".$lang."'
    WHERE properties.pt_visibility = 1 AND properties.pt_sold = 0 AND tr_properties.tr_lang = '".$lang."' GROUP BY properties.pt_id ORDER BY properties.pt_date DESC LIMIT $value"); 
    $sentence->execute();
    return $sentence->fetchAll();
}

function getPropertyById($connect, $id_property, $lang){
    $sentence = $connect->prepare("SELECT properties.*, tr_properties.*, tr_ptstatus.tr_name AS status, tr_ptcities.tr_name AS city, tr_ptzones.tr_name AS zone, tr_pttypes.tr_name AS type, tr_ptconditions.tr_name AS conditions, tr_properties.tr_slug AS slug FROM properties LEFT JOIN tr_properties ON properties.pt_id = tr_properties.tr_property LEFT JOIN tr_ptstatus ON properties.pt_status = tr_ptstatus.tr_status AND tr_ptstatus.tr_lang = '".$lang."' LEFT JOIN tr_pttypes ON properties.pt_type = tr_pttypes.tr_type AND tr_pttypes.tr_lang = '".$lang."' LEFT JOIN tr_ptcities ON properties.pt_city = tr_ptcities.tr_city AND tr_ptcities.tr_lang = '".$lang."' LEFT JOIN tr_ptzones ON properties.pt_zone = tr_ptzones.tr_zone AND tr_ptzones.tr_lang = '".$lang."' LEFT JOIN tr_ptconditions ON properties.pt_conditions = tr_ptconditions.tr_conditions AND tr_ptconditions.tr_lang = '".$lang."' WHERE properties.pt_visibility = 1 AND tr_properties.tr_lang = '".$lang."' AND properties.pt_id = '".$id_property."' LIMIT 1"); 
    $sentence->execute();
    $row = $sentence->fetch();
    return $row;
}
...
function getUserFavorites($connect, $userId){
    $sentence = $connect->prepare("SELECT properties.*, tr_properties.*, tr_ptstatus.tr_name AS status, tr_ptcities.tr_name AS city, tr_ptzones.tr_name AS zone, tr_pttypes.tr_name AS type, tr_ptconditions.tr_name AS conditions, tr_properties.tr_slug AS slug, favorites.* FROM favorites, properties LEFT JOIN tr_properties ON properties.pt_id = tr_properties.tr_property LEFT JOIN tr_ptstatus ON properties.pt_status = tr_ptstatus.tr_status LEFT JOIN tr_pttypes ON properties.pt_type = tr_pttypes.tr_type LEFT JOIN tr_ptcities ON properties.pt_city = tr_ptcities.tr_city LEFT JOIN tr_ptconditions ON properties.pt_conditions = tr_ptconditions.tr_conditions LEFT JOIN tr_ptzones ON properties.pt_zone = tr_ptzones.tr_zone WHERE favorites.user = '".$userId."' AND favorites.item = properties.pt_id ");
    $sentence->execute();
    return $sentence->fetchAll();
}
...
function getShortListedPropertyPage($connect){
    $sentence = $connect->prepare("SELECT pages.page_id, tr_pages.tr_slug, tr_pages.tr_title FROM pages, tr_pages  WHERE pages.page_id = tr_pages.tr_page AND pages.page_visibility = 1  AND pages.page_id = 16 AND tr_pages.tr_page = 16 AND tr_pages.tr_lang = 'en' LIMIT 1");
    $sentence->execute();
    $row = $sentence->fetch();
...
function isMobile() {
    // List of common mobile device identifiers
    $mobileDevices = ['Mobile', 'Android', 'iPhone', 'iPad', 'Windows Phone', 'BlackBerry', 'webOS'];

    // Get the User-Agent string
    $userAgent = $_SERVER['HTTP_USER_AGENT'];

    // Check if the User-Agent contains any of the mobile device identifiers
    ...
    return false; // It's not a mobile device (assuming desktop)
}

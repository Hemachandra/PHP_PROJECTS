...
}elseif(!lengthInput($user_name, 3, 50)){
...
if (empty($user_phone)) {
    $errors[] = $translation['tr_140'].' is required';
}elseif(!lengthInput($user_phone, 10, 10)){
	$errors[] = 'Phone number must be 10 digit valid';
}else{
	$validatePassword = true;
}
...
if (empty($errors)) {
      ...
      $statement = $connect->prepare("INSERT INTO users (user_id, user_name, user_email, user_password, user_phone) VALUES (null, :user_name, :user_email, :user_password, :user_phone)");

...
if ($_POST){
    ...
    $pageName = filter_var(strtolower(isset($_POST['pageName'])?($_POST['pageName']):('')), FILTER_SANITIZE_STRING);
    $url = filter_var(strtolower(isset($_POST['url'])?($_POST['url']):('')), FILTER_SANITIZE_STRING);
        ...
        if ($emailTemplate) {
            ...
            try{        
                ...
                $errors[] = $e->getMessage();   
                            ...
                            $statement = $connect->prepare("INSERT INTO property_lead (user_id, name, email,phone,message,url,ref,type) VALUES (:user_id, :name, :email,:phone,:message,:url,:ref,:type)");

...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	$pt_zone = cleardata($_POST['pt_zone']??NULL);
	...
	$pt_reference = cleardata($_POST['pt_reference']??'0');
	$pt_featured = cleardata($_POST['pt_featured']??'0');
	$pt_direction = $_POST['pt_direction']??0;
	$pt_visibility = cleardata($_POST['pt_visibility']);
	$pt_floor = cleardata($_POST['pt_floor']??0);
	...
	$pt_sold = cleardata($_POST['pt_sold']??'0');
	...
	$statment->execute(array(
		...
		':pt_video' => $pt_video,
		':pt_image' => $pt_image,
		':top_features' => json_encode($features,true), // Store as JSON
...
// get uploaded files
	...
	set_msg('success','Property information has been updated successfully!');
	...
	$village = get_all_village_by_mandal($connect, $property['pt_mandal']);

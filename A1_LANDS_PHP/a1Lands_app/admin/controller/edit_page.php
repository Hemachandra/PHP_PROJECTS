...
if (isset($_SESSION["user_email"])) {
    ...
    if ($check_access["user_role"] == 1) {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $brochure = new FileUploader("brochure", [
                "uploadDir" => "../../images/",
                "title" => "auto",
                "maxSize" => 4,
                "fileMaxSize" => 4,
            ...
            $sentence = $connect->prepare(
                "UPDATE tr_pages SET tr_title = :tr_title, tr_content = :tr_content, tr_seotitle = :tr_seotitle, tr_seodescription = :tr_seodescription, tr_slug = :tr_slug WHERE tr_id = :tr_id AND tr_lang = :tr_lang AND tr_page = :tr_page"
            );
            ...
            $databrochure = $brochure->upload();
          
            if (
                $databrochure["isSuccess"] &&
                count($databrochure["files"]) > 0
            ) {
                $brochure = $databrochure["files"][0]["name"];
                $sentence = $connect->prepare(
                    "UPDATE pages SET icon = :icon WHERE page_id = :page_id"
                );

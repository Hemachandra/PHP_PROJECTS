...
function set_msg($type,$msg){
    $_SESSION['flash_message'] = array("msg" => $msg, "type" => $type);
}
...
function isSubscriber($connect){

    if (isset($_SESSION['user_email'])) {

    $emailSession = filter_var(strtolower($_SESSION['user_email']), FILTER_SANITIZE_STRING);
    
    $sentence = $connect->prepare("SELECT * FROM users WHERE user_email = '".$emailSession."' AND user_status = 1 AND user_role = 3 LIMIT 1"); 
    $sentence->execute();
    $row = $sentence->fetch();
...
function get_property_types_languages($connect)
{
    
    $sentence = $connect->prepare("SELECT * FROM languages WHERE language_status = 1 AND language_code IN (SELECT tr_lang FROM tr_pttypes_pro) ORDER BY language_id ASC"); 
    $sentence->execute();
    return $sentence->fetchAll();
}
...
function totalPropertyTypes($connect, $items_per_page){

    $total_items = $connect->prepare("SELECT COUNT(*) AS total FROM pt_types_pro");
    $total_items->execute();
    $total_items = $total_items->fetch()['total'];
...
function get_all_property_types($connect, $items_per_page = NULL )
{
    $limit = (currentPage() > 1) ? currentPage() * $items_per_page - $items_per_page : 0;
    
    $sql = "SELECT pt_types_pro.*,tr_pttypes_pro.tr_name AS tr_name FROM pt_types_pro,tr_pttypes_pro,settings WHERE pt_types_pro.pt_type_id = tr_pttypes_pro.tr_type AND tr_pttypes_pro.tr_lang = settings.st_language GROUP BY tr_pttypes_pro.tr_type";
...
function get_languages_by_property_type($connect, $id_type)
{
    
    $sentence = $connect->prepare("SELECT * FROM languages WHERE language_status = 1 AND language_code IN (SELECT tr_lang FROM tr_pttypes_pro WHERE tr_type = $id_type) ORDER BY language_id ASC"); 
    $sentence->execute();
    return $sentence->fetchAll();
}
...
function get_property_type_per_id_by_language($connect, $id_type, $lang){
    $sentence = $connect->query("SELECT pt_types_pro.*,tr_pttypes_pro.* FROM pt_types_pro,tr_pttypes_pro WHERE pt_types_pro.pt_type_id = $id_type AND tr_pttypes_pro.tr_type = $id_type AND tr_pttypes_pro.tr_lang = '".$lang."' LIMIT 1");
    $sentence = $sentence->fetchAll();
    return ($sentence) ? $sentence : false;
}
...
function check_property_type($connect, $id_type, $lang){
    $sentence = $connect->query("SELECT * FROM tr_pttypes_pro WHERE tr_type = $id_type AND tr_lang = '".$lang."' LIMIT 1");
    $sentence = $sentence->fetchAll();
    return ($sentence) ? $sentence : false;
}
...
function get_languages_not_property_type($connect, $id_type)
{
    
    $sentence = $connect->prepare("SELECT * FROM languages WHERE language_status = 1 AND language_code NOT IN (SELECT tr_lang FROM tr_pttypes_pro WHERE tr_type = $id_type) ORDER BY language_id ASC"); 
    $sentence->execute();
    return $sentence->fetchAll();
}
...
function get_property_type_slug($connect, $slug){

    $sentence = $connect->prepare("SELECT COUNT(*) AS total FROM tr_pttypes_pro WHERE tr_slug LIKE '$slug%'");
    $sentence->execute();
    $row = $sentence->fetch(PDO::FETCH_ASSOC);
    return $row['total'];
}
...
function get_all_properties($connect, $items_per_page = NULL)
{
    ...
    if(isSubscriber($connect)){
        $where .= " AND user_id='".$_SESSION['user_id']."'";
    }
    $sql = "SELECT properties.*,tr_properties.* FROM properties,tr_properties,settings WHERE properties.pt_id = tr_properties.tr_property $where AND tr_properties.tr_lang = settings.st_language GROUP BY tr_properties.tr_property ORDER BY pt_id DESC";
...
function get_all_leads($connect)
{
    
    $sentence = $connect->prepare("SELECT * FROM leads ORDER BY leads.id DESC"); 
    $sentence->execute();
    return $sentence->fetchAll();
}
...
function get_mandal_languages($connect)
{
    
    $sentence = $connect->prepare("SELECT * FROM languages WHERE language_status = 1 AND language_code IN (SELECT tr_lang FROM tr_ptmandal) ORDER BY language_id ASC"); 
    $sentence->execute();
    return $sentence->fetchAll();
}
...
function totalMandal($connect, $items_per_page){

    $total_items = $connect->prepare("SELECT COUNT(*) AS total FROM pt_mandal");
    $total_items->execute();
    $total_items = $total_items->fetch()['total'];
...
function get_all_mandal($connect, $items_per_page = NULL)
{
    
    $limit = (currentPage() > 1) ? currentPage() * $items_per_page - $items_per_page : 0;
    
    $sql = "SELECT pt_mandal.*,tr_ptmandal.tr_name AS tr_name FROM pt_mandal,tr_ptmandal,settings WHERE pt_mandal.pt_mandal_id = tr_ptmandal.tr_mandal AND tr_ptmandal.tr_lang = settings.st_language GROUP BY pt_mandal.pt_mandal_id";
...
function id_mandal($id_city){
    return (int)cleardata($id_city);
}

function get_languages_by_mandal($connect, $id_city)
{
    
    $sentence = $connect->prepare("SELECT * FROM languages WHERE language_status = 1 AND language_code IN (SELECT tr_lang FROM tr_ptmandal WHERE tr_mandal = $id_city) ORDER BY language_id ASC"); 
    $sentence->execute();
    return $sentence->fetchAll();
}

function get_mandal_per_id_by_language($connect, $id_city, $lang){
    $sentence = $connect->query("SELECT pt_mandal.*,tr_ptmandal.* FROM pt_mandal,tr_ptmandal WHERE pt_mandal.pt_mandal_id = $id_city AND tr_ptmandal.tr_mandal = $id_city AND tr_ptmandal.tr_lang = '".$lang."' LIMIT 1");
    $sentence = $sentence->fetchAll();
    return ($sentence) ? $sentence : false;
}

function check_mandal($connect, $id_city, $lang){
    $sentence = $connect->query("SELECT * FROM tr_ptmandal WHERE tr_mandal = $id_city AND tr_lang = '".$lang."' LIMIT 1");
    $sentence = $sentence->fetchAll();
    return ($sentence) ? $sentence : false;
}


function get_languages_not_mandal($connect, $id_city)
{
    
    $sentence = $connect->prepare("SELECT * FROM languages WHERE language_status = 1 AND language_code NOT IN (SELECT tr_lang FROM tr_ptmandal WHERE tr_mandal = $id_city) ORDER BY language_id ASC"); 
    $sentence->execute();
    return $sentence->fetchAll();
}

function get_mandal_slug($connect, $slug){

$sentence = $connect->prepare("SELECT COUNT(*) AS total FROM tr_ptmandal WHERE tr_slug LIKE '$slug%'");
$sentence->execute();
$row = $sentence->fetch(PDO::FETCH_ASSOC);
return $row['total'];
}
...
function get_village_languages($connect)
{
    
    $sentence = $connect->prepare("SELECT * FROM languages WHERE language_status = 1 AND language_code IN (SELECT tr_lang FROM tr_ptvillage) ORDER BY language_id ASC"); 
    $sentence->execute();
    return $sentence->fetchAll();
}


function totalVillage($connect, $items_per_page){

    $total_items = $connect->prepare("SELECT COUNT(*) AS total FROM pt_village");
    $total_items->execute();
    $total_items = $total_items->fetch()['total'];
...
function get_all_village($connect, $items_per_page = NULL)
{
    
    $limit = (currentPage() > 1) ? currentPage() * $items_per_page - $items_per_page : 0;
    
    $sql = "SELECT pt_village.*,tr_ptvillage.tr_name AS tr_name,tr_ptmandal.tr_name AS mandal_name 
    
    FROM pt_village,tr_ptvillage,tr_ptmandal,settings 
    
    WHERE pt_village.pt_village_id = tr_ptvillage.tr_village 
    AND pt_village.pt_village_mandal = tr_ptmandal.tr_mandal
    AND tr_ptvillage.tr_lang = settings.st_language 
    AND tr_ptvillage.tr_lang = settings.st_language 
    GROUP BY pt_village.pt_village_id";
...
function get_all_village_by_mandal($connect, $id_city)
{
    
    
    $sql = "SELECT pt_village.*,tr_ptvillage.tr_name AS tr_name FROM pt_village,tr_ptvillage WHERE pt_village.pt_village_id = tr_ptvillage.tr_village AND pt_village.pt_village_mandal = $id_city GROUP BY pt_village.pt_village_id";
...
function id_village($id_zone){
    return (int)cleardata($id_zone);
}

function get_languages_by_village($connect, $id_zone)
{
    
    $sentence = $connect->prepare("SELECT * FROM languages WHERE language_status = 1 AND language_code IN (SELECT tr_lang FROM tr_ptvillage WHERE tr_village = $id_zone) ORDER BY language_id ASC"); 
    $sentence->execute();
    return $sentence->fetchAll();
}

function get_village_per_id_by_language($connect, $id_zone, $lang){
    $sentence = $connect->query("SELECT pt_village.*,tr_ptvillage.*,tr_ptmandal.tr_name AS mandal_name FROM pt_village,tr_ptvillage,tr_ptmandal WHERE pt_village.pt_village_id = $id_zone AND pt_village.pt_village_mandal = tr_ptmandal.tr_mandal AND tr_ptvillage.tr_village = $id_zone AND tr_ptvillage.tr_lang = '".$lang."' AND tr_ptmandal.tr_lang = '".$lang."' LIMIT 1");
    $sentence = $sentence->fetchAll();
    return ($sentence) ? $sentence : false;
}

function check_village($connect, $id_zone, $lang){
    $sentence = $connect->query("SELECT * FROM tr_ptvillage WHERE tr_village = $id_zone AND tr_lang = '".$lang."' LIMIT 1");
    $sentence = $sentence->fetchAll();
    return ($sentence) ? $sentence : false;
}


function get_languages_not_village($connect, $id_zone)
{
    
    $sentence = $connect->prepare("SELECT * FROM languages WHERE language_status = 1 AND language_code NOT IN (SELECT tr_lang FROM tr_ptvillage WHERE tr_village = $id_zone) ORDER BY language_id ASC"); 
    $sentence->execute();
    return $sentence->fetchAll();
}

function get_village_slug($connect, $slug){

$sentence = $connect->prepare("SELECT COUNT(*) AS total FROM tr_ptvillage WHERE tr_slug LIKE '$slug%'");
$sentence->execute();
$row = $sentence->fetch(PDO::FETCH_ASSOC);
return $row['total'];
}
...
function get_all_sliders($connect)
{
    ...
    $sentence = $connect->prepare("SELECT sliders.*, properties.pt_reference AS property_reference 
    FROM sliders 
    LEFT JOIN properties ON sliders.slider_property = properties.pt_id 
    ORDER BY sliders.slider_id DESC");
$sentence->execute();
    return $sentence->fetchAll();
}
